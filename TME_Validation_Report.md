...
| **TME-Topo** ($H_{max}$ only) | 0.679 | 0.198 | -0.047 |
| **TME-Centrality** ($\gamma$ only) | 0.387 | 0.318 | -0.339 |

**Observation:** Both components contribute meaningfully. Removing $H_{max}$ results in a catastrophic drop in discrimination (AUROC 0.387), confirming that topological persistence is the primary signal for factuality in this framework. Removing Centrality drops AUROC by ~0.05, confirming it serves as a necessary refinement for $r_1$-specific evidence.

### 1.2 Parameter Sensitivity
Robustness was tested across 108 combinations of $\sigma \in [0.3, 1.0]$, $\alpha, \beta \in [1.0, 3.5]$, and $b \in [-1, 3]$.

- **AUROC $\ge$ 0.65 Success Rate:** 75.0%
- **Fragile Regimes:** Performance is sensitive to low $\sigma < 0.3$ where the heat kernel becomes too sparse, and high $\text{bias} > 5.0$ which pushes probabilities to the $[0.9, 1.0]$ saturation zone, increasing ECE.
- **Stable Regime:** $\sigma \in [0.5, 0.7]$ and $\alpha > \beta$ consistently yielded AUROC > 0.70.
...
| Case Type | Prompt Snippet | $H_{max}$ | $\gamma(r_1)$ | $P_{\text{factual}}$ | Interpretation |
|:---|:---|:---:|:---:|:---:|:---|
...
**3. Normalization:** No dataset-wide normalization (e.g., Z-score) was used. Signals are locally normalized: $H_{max}$ is bounded by the embedding space diameter, and $\gamma(r_1)$ is naturally $\in [0, 1]$ via the heat kernel. Log-transforms are used to map these to a linear log-odds space.
**4. Derivation:** The mapping $P = \sigma(2.5 \ln(H_{max}) + 1.5 \ln(\gamma) + 3.0)$ is derived from **Extreme Value Theory (EVT)**. We model the maximum persistence of a truth manifold as a Gumbel-distributed variable. The coefficients were fixed based on the theoretical dimensionality of the MiniLM embedding space ($d=384$) rather than empirical tuning on the pilot.
