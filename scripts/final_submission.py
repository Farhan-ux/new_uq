...
def main():
    ...
    # Summary Report
    derivation = """# Mathematical Derivation: Topological Manifold Evidence (TME)
...
1. **Topological Persistence ($H_0$):** We compute the 0-dimensional persistent homology
   ...
   resulting in a robust manifold ($high H_{max}$), whereas adversarial hallucinations
   are often singular or collapsed ($low H_{max}$).
...
3. **Probability Mapping:** The log-odds of factuality are modeled as a linear combination
   ...
   $P(\text{factual}) = \sigma( \alpha \ln(H_{max}) + \beta \ln(\gamma(r_1)) + \gamma )$
   The parameters $\alpha, \beta, \gamma$ are derived based on the assumption that factual
