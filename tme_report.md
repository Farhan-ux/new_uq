...
2. **Evidential Centrality:** We measure the support for $r_1$ via a heat kernel:
   $\gamma(r_1) = rac{1}{N} \sum_{j=1}^N \exp\left(-rac{\|e_1 - e_j\|^2}{2\sigma^2}ight)$
...
3. **Probability Mapping:** The log-odds of factuality are modeled as a linear combination
   ...
   $P(	ext{factual}) = \sigma( lpha \ln(H_{max}) + eta \ln(\gamma(r_1)) + \gamma )$
   The parameters $lpha, eta, \gamma$ are derived based on the assumption that factual
